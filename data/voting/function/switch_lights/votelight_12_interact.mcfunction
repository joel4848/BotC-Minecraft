...
execute unless score #voudon temp matches 1 run execute at @e[type=marker,tag=votelight12,limit=1,sort=nearest] run playsound minecraft:block.lever.click block @a ~ ~ ~ 1.0 1.0
execute as @e[type=marker,tag=votelight12,limit=1,sort=nearest] at @s if score #voudon temp matches 1 run execute if block ~ ~ ~ waxed_weathered_copper_bulb run playsound minecraft:block.lever.click block @a ~ ~ ~ 1.0 1.0

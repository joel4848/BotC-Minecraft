...
execute at @e[tag=votelight3,limit=1,sort=nearest] if score @a[scores={Player=-3},limit=1] Alive matches 0 if score @a[scores={Player=-3},limit=1] IsBanshee matches 1 run execute unless score #og_silence_votelights temp matches 1 run playsound minecraft:block.lever.click block @a ~ ~ ~ 1.0 1.0
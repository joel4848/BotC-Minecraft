...
team leave @a[scores={Player=-1}]
team empty Nominated
team join Nominated @a[scores={Player=-1}]
scoreboard players set @a[scores={Player=-1}] Nominated 1
effect give @a[scores={Player=-1}] glowing infinite 99 true
...
#  function clock_hand:big_hand/rotate_to_1 with storage joelbotc

scoreboard players set #big_hand_target temp 1

scoreboard players set #nominated_player_big_hand temp 1

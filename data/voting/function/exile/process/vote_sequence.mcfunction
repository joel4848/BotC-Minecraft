...
schedule function voting:exile/process/remove_lever 5t

# Wait for piston to fully retract, then count the vote

schedule function voting:exile/process/count_vote 10t
...
execute unless score #current_seat temp = #nominated temp run execute if score #current_player temp < #player_count temp run schedule function voting:exile/process/vote_sequence 25t
